...
func get_value_string() -> String:
	match _data_type:
		...
		Data.Type.BITFLAGS:
			return Utils.flags_to_string(_getter.call(), _enum_dict)
		
		Data.Type.INPUTEVENT:
			var event: InputEvent = _getter.call()
			return event.as_text() if is_instance_valid(event) else ""
