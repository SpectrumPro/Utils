...
func _init() -> void:
	...
	if LoggerConfig.enable_remote_logging:
...
## Stores config for logger
class LoggerConfig extends Object:
	## Enables remote logging
	static var enable_remote_logging: bool = false
	...
	static func load_config(p_path: String) -> bool:
		...
		enable_remote_logging = type_convert(config.get("enable_remote_logging", enable_remote_logging), TYPE_BOOL)
